...
public class VideoConfigActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        System.out.println("config ----seting----");
        setContentView(R.layout.activity_video_config);
        System.out.println("config --------------=="+ Color.BLACK);
        InitialLayout();
    }
    private void InitialLayout() {
        ...
        System.out.println("color---state---"+Color.BLACK);
        mEnableP2PBox.setChecked(mConfigEntity.mEnableP2P != 0);
        System.out.println("color---state-end--");
        // Overlay视频模式
        mVideoOverlayBox = (CheckBox)this.findViewById(R.id.enable_overLay_box);
        mVideoOverlayBox.setTextColor(Color.BLACK);
        mVideoOverlayBox.setChecked(mConfigEntity.mVideoOverlay !=0);

        // 翻转视频
        mVideoRotateBox = (CheckBox)this.findViewById(R.id.enable_video_rotate);
        mVideoRotateBox.setTextColor(Color.BLACK);
        mVideoRotateBox.setChecked(mConfigEntity.mVideoOverlay !=0);

        // 本地视频采集偏色修正
        mFixColorDeviation = (CheckBox)this.findViewById(R.id.enable_fix_color_box);
        mFixColorDeviation.setTextColor(Color.BLACK);
        mFixColorDeviation.setChecked(mConfigEntity.mFixColorDeviation !=0);

        // 启用视频GPU渲染
        mVideoShowGPURender = (CheckBox)this.findViewById(R.id.enable_GPU_render);
        mVideoShowGPURender.setTextColor(Color.BLACK);
        mVideoShowGPURender.setChecked(mConfigEntity.mFixColorDeviation !=0);
        ...
        // 强制使用ARMv6指令集（安全模式）
        mUseARMv6Box = (CheckBox)this.findViewById(R.id.enable_use_armV6);
        mUseARMv6Box.setTextColor(Color.BLACK);
        mUseARMv6Box.setChecked(mConfigEntity.mUseARMv6Lib !=0);

        // 启用回音消除（AEC）
        mUseAECBox = (CheckBox)this.findViewById(R.id.enable_use_AEC);
        mUseAECBox.setTextColor(Color.BLACK);
        mUseAECBox.setChecked(mConfigEntity.mEnableAEC !=0);

        // 启用平台内置硬件编解码（需重启应用程序）
        mUseHWCodecBox = (CheckBox)this.findViewById(R.id.enable_use_hardware_code);
        mUseHWCodecBox.setTextColor(Color.BLACK);
        mUseHWCodecBox.setChecked(mConfigEntity.mUseHWCodec !=0);


        //mode
        mServerModelConfigBtn = (RadioButton)findViewById(R.id.serverConfig);
        mCustomModelConfigBtn = (RadioButton)findViewById(R.id.customerConfig);
        ...
        //选择视频分辨率：
        // 插入视频分辨率
        mVideoSizeSpinner = (Spinner)this.findViewById(R.id.videoResolution);
    ...
    private Spinner InsertSpinnerInterface(int spinnerIndex, String[] context, int[] value, int select) {
        ...
        if (spinnerIndex == 1){
            spinner = (Spinner)this.findViewById(R.id.videoBitrate);
        }else if (spinnerIndex == 2) {
            spinner = (Spinner)this.findViewById(R.id.videoFPS);
        }else if (spinnerIndex == 3) {
            spinner = (Spinner)this.findViewById(R.id.videoQuality);
        }else if (spinnerIndex == 4) {
            spinner = (Spinner)this.findViewById(R.id.videoprepremeter);
        }
